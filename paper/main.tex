...
\begin{figure}[h]
  ...
  \fbox{\parbox{0.8\linewidth}{
    Diagram placeholder: IndexedDB database \texttt{zktv} v1 with stores
    \texttt{vaults} (PK \texttt{[vaultId, scheme]} $\to$ serialized \texttt{WrappedSecretRecord}, \texttt{updatedAt}),
    \texttt{credentials} (PK \texttt{credentialId}, index \texttt{vaultId} $\to$ \texttt{counter}, \texttt{prfSalt}, \texttt{transports}, \texttt{createdAt}), and
    \texttt{meta} (PK \texttt{key}). Arrow: \texttt{credentials.vaultId} $\to$ \texttt{vaults.vaultId} (1 vault : N credentials).
  }}
